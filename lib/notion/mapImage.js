...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // 处理 Notion custom_emoji URL 格式
  if (img.startsWith('notion://custom_emoji/')) {
    // 将 notion://custom_emoji/ 转换为 https://www.notion.so/custom_emoji/
    ret = img.replace('notion://custom_emoji/', 'https://www.notion.so/custom_emoji/')
  ...
  const hasConverted =
     ...
     ret.includes('https://www.notion.so/custom_emoji/')
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0 && !ret.includes('custom_emoji')) {
...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  if (image.includes('custom_emoji')) {
    return image
  }
